...
" added by chud - save all windows' sizes before zoom-in and
" use 'exe s:zoom_allWinSize' to restore all windows' sizes
" --- don't need to give it a value at the start
" let s:zoom_allWinSize = winrestcmd()
...
function! s:Tlist_Window_Zoom()
    if s:tlist_win_maximized
        ...
        " chud - directly use 'exe s:zoom_allWinSize' (2 times to ensure the
        " effect) and no need to use ifs
        exe s:zoom_allWinSize
        exe s:zoom_allWinSize
        " if g:Tlist_Use_Horiz_Window
        ...
        " windows
        " chud - first save all windows' sizes
        let s:zoom_allWinSize = winrestcmd()
        if g:Tlist_Use_Horiz_Window
            resize
        else
...
function! s:Tlist_Window_Close()
    " chud - if it's maximized when exit, first restore all windows' position
    " so that it won't change their positions
    if s:tlist_win_maximized
...
function! s:Tlist_Window_Open()
    ...
    " " chud - let the flag be 0 to fix the bug that zoom doen't work
    " " at the 1st 'x' if close it when maximized - it works
    " let s:tlist_win_maximized = 0
    " --- don't need this as fix(restore windows first) added at Close part
